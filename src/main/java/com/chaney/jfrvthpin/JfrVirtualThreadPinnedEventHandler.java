...
class JfrVirtualThreadPinnedEventHandler {
    ...
    private String formatStackTrace(RecordedStackTrace stackTrace) {
        ...
        String formatted = "\t" + stackTrace.getFrames().stream()
                .limit(JfrVirtualThreadPinnedEventHandler.STACK_TRACE_MAX_DEPTH)
                .map(JfrVirtualThreadPinnedEventHandler::formatStackTraceFrame)
                .collect(Collectors.joining("\n\t"));
        if (JfrVirtualThreadPinnedEventHandler.STACK_TRACE_MAX_DEPTH < stackTrace.getFrames().size()) {
            return formatted + "\n\t(...)"; // truncated
        }
        return formatted;
    }
